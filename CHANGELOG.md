...
- **Sound & Light Machine transport rewritten**, ported from [nanit-sound-light](https://github.com/com6056/nanit-sound-light) as the first stage of the merge planned in #102. Commands now send one request at a time and wait for the device's acknowledgement, multi-entity changes (a scene touching power, sound, volume, and light at once) coalesce into a single combined write, and commanded values are briefly pinned so the device's lagging state reports can't flip a just-issued command back. Sends wait for the cloud relay to report the speaker attached. Together these remove the command latency and the occasional wedged speaker (unresponsive until power-cycled) that the fire-and-forget path could produce.
...
- A session re-initialization that failed after a reconnect is now finished by the next health check when the socket stayed up. The failure log always promised that retry, but it only actually happened after another connection drop, and until then sensor push stayed disabled with motion and sound degraded to the slow poll.
- Adding an account that is already configured now aborts before attempting the login, so a duplicate add no longer burns an authentication round trip or, on MFA accounts, sends a code for a flow that can only end in already_configured.
