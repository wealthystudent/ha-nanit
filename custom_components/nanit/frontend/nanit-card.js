...
`;let bt=class extends ot{setConfig(t){this._config={...t}}_entityChanged(t){const e=t.detail.value;if(!this._config||e===this._config.camera_entity_id)return;const i={...this._config,camera_entity_id:e};this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{bubbles:!0,composed:!0,detail:{config:i}}))}render(){return this.hass&&this._config?B`
  ...
  `,t([pt({attribute:!1})],bt.prototype,"hass",void 0),t([ut()],bt.prototype,"_config",void 0),bt=t([lt("nanit-card-editor")],bt);let ft=class extends ot{constructor(){super(...arguments),this._streamLoaded=!1}static getConfigElement(){return document.createElement("nanit-card-editor")}static getStubConfig(t){const e=Object.keys(t.states).find(e=>e.startsWith("camera.")&&"nanit"===t.entities[e]?.platform);return{type:"custom:nanit-card",camera_entity_id:e||""}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config=t}getCardSize(){return 5}_entities(){return this._config?.camera_entity_id&&this.hass?function(t,e){const i={};i.camera=e;const r=t.entities[e];if(r?.device_id){const e=r.device_id,s=[];for(const i of Object.keys(t.entities))t.entities[i].device_id===e&&"diagnostic"!==t.entities[i].entity_category&&s.push(i);mt(i,s,t)}else{const r=e.split(".")[1]??"";mt(i,Object.keys(t.states).filter(t=>t!==e&&t.split(".")[1]?.startsWith(r.split("_camera")[0]||r)),t)}return i}(this.hass,this._config.camera_entity_id):{}}_isCameraOn(t){return!t.power||"on"===this.hass.states[t.power]?.state}_fireMoreInfo(t){this.dispatchEvent(new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:t}}))}_toggleService(t,e,i){this.hass.callService(t,e,{entity_id:i})}disconnectedCallback(){super.disconnectedCallback(),this._clearStreamCheck()}updated(t){if(super.updated(t),!this._streamLoaded&&!this._streamCheckTimer){this.renderRoot.querySelector("ha-camera-stream")&&this._scheduleStreamCheck()}}_scheduleStreamCheck(){this._streamCheckTimer=setTimeout(()=>{this._streamCheckTimer=void 0;const t=this.renderRoot.querySelector("ha-camera-stream");if(!t)return;const e=t.shadowRoot;e?.querySelector("video, img, canvas")?this._streamLoaded=!0:this._scheduleStreamCheck()},500)}_clearStreamCheck(){this._streamCheckTimer&&(clearTimeout(this._streamCheckTimer),this._streamCheckTimer=void 0)}_onStreamLoad(){this._streamLoaded=!0,this._clearStreamCheck()}render(){if(!this.hass||!this._config)return B`<ha-card><div class="header"><span class="device-name">Nanit</span></div></ha-card>`;const t=this._entities(),e=this._isCameraOn(t);!e&&this._streamLoaded&&(this._streamLoaded=!1,this._clearStreamCheck());const i=t.camera?function(t,e){const i=t.states[e];return i?(i.attributes.friendly_name??"Nanit").replace(/ Camera$/i,""):"Nanit"}(this.hass,t.camera):"Nanit";return B`
      <ha-card>
    ...
    `}_renderHeader(t,e,i){return B`
      <div class="header">
        ...
        ${i?F:B`<span class="camera-off-label">Camera Off</span>`}
        ${e.power?B`
              <button
    ...
    `}_renderSensorOverlays(t){const e=[];if(gt(this.hass,t.temperature)){const i=parseFloat(this.hass.states[t.temperature].state),r=isNaN(i)?this.hass.states[t.temperature].state:i.toFixed(1),s=this.hass.states[t.temperature].attributes.unit_of_measurement??"";e.push(B`
        <div class="pill pill-temp" @click=${()=>this._fireMoreInfo(t.temperature)}>
          <ha-icon icon="mdi:thermometer"></ha-icon>
          <span>${r}${s}</span>
        </div>
      `)}if(gt(this.hass,t.humidity)){const i=parseFloat(this.hass.states[t.humidity].state),r=isNaN(i)?this.hass.states[t.humidity].state:i.toFixed(1);e.push(B`
        <div class="pill pill-humid" @click=${()=>this._fireMoreInfo(t.humidity)}>
          <ha-icon icon="mdi:water-percent"></ha-icon>
          <span>${r}%</span>
        </div>
      `)}if(gt(this.hass,t.light)){const i=parseFloat(this.hass.states[t.light].state),r=isNaN(i)?this.hass.states[t.light].state:Math.round(i).toString();e.push(B`
        <div class="pill pill-light" @click=${()=>this._fireMoreInfo(t.light)}>
          <ha-icon icon="mdi:brightness-5"></ha-icon>
          <span>${r} lx</span>
        </div>
      `)}return 0===e.length?B``:B`<div class="overlay-top">${e}</div>`}_renderDetectionOverlays(t){const e=gt(this.hass,t.motion),i=gt(this.hass,t.sound);if(!e&&!i)return B``;const r=e&&"on"===this.hass.states[t.motion].state,s=i&&"on"===this.hass.states[t.sound].state;return B`
    ...
    `}_renderSoundMachine(t){const e=this.hass.states[t],i="playing"===e?.state,r=e?.attributes.source??"",s=e?.attributes.source_list??[],n=e?.attributes.volume_level??0,a=Math.round(100*n);return B`
      <div class="control-section control-section-sound">
        ...
        <div class="control-row">
          ...
          ${i?B`<span class="track-name">${this._formatSourceName(r)}</span>`:F}
          <div class="slider-row">
