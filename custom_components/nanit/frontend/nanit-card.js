...
`;let yt=class extends ot{setConfig(t){this._config={...t}}_entityChanged(t,e){const i=e.detail.value||void 0;this._config&&i!==this._config[t]&&this._updateConfig({[t]:i})}_toggleChanged(t,e){const i=e.target.checked;this._updateConfig({[t]:i})}_updateConfig(t){const e={...this._config,...t};for(const[i,s]of Object.entries(t))void 0===s&&delete e[i];this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{bubbles:!0,composed:!0,detail:{config:e}}))}render(){return this.hass&&this._config?q`
  ...
  `,t([pt({attribute:!1})],yt.prototype,"hass",void 0),t([ut()],yt.prototype,"_config",void 0),yt=t([lt("nanit-card-editor")],yt);let $t=class extends ot{constructor(){super(...arguments),this._streamLoaded=!1,this._showNetwork=!1,this._streamEpoch=0,this._streamCheckCount=0,this._streamRetryCount=0}static getConfigElement(){return document.createElement("nanit-card-editor")}static getStubConfig(t){const e=Object.keys(t.states).find(e=>e.startsWith("camera.")&&"nanit"===t.entities[e]?.platform);return{type:"custom:nanit-card",camera_entity_id:e||""}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config=t}getCardSize(){return 5}_entities(){return this._config?.camera_entity_id&&this.hass?function(t,e){const i={};i.camera=e;const s=t.entities[e];if(s?.device_id){const e=s.device_id,n=[],r=[];for(const i of Object.keys(t.entities))t.entities[i].device_id===e&&("diagnostic"===t.entities[i].entity_category?r.push(i):n.push(i));vt(i,n,t),function(t,e,i){for(const s of e){const[e]=s.split(".",1),n=s.split(".")[1]??"",r=i.states[s]?.attributes.device_class;"sensor"===e&&("signal_strength"===r?t.wifi_signal=s:"frequency"===r?t.wifi_frequency=s:n.endsWith("_wifi_ssid")&&(t.wifi_ssid=s))}}(i,r,t)}else{const s=e.split(".")[1]??"";vt(i,Object.keys(t.states).filter(t=>t!==e&&t.split(".")[1]?.startsWith(s.split("_camera")[0]||s)),t)}return i}(this.hass,this._config.camera_entity_id):{}}_isCameraOn(t){return!t.power||"on"===this.hass.states[t.power]?.state}_fireMoreInfo(t){this.dispatchEvent(new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:t}}))}_toggleService(t,e,i){this.hass.callService(t,e,{entity_id:i})}disconnectedCallback(){super.disconnectedCallback(),this._clearStreamCheck()}updated(t){if(super.updated(t),!this._streamLoaded&&!this._streamCheckTimer){this.renderRoot.querySelector("ha-camera-stream")&&this._scheduleStreamCheck()}}_scheduleStreamCheck(){this._streamCheckTimer=setTimeout(()=>{this._streamCheckTimer=void 0;const t=this.renderRoot.querySelector("ha-camera-stream");if(!t)return;const e=t.shadowRoot;e?.querySelector("video, img, canvas")?(this._streamLoaded=!0,this._streamCheckCount=0):++this._streamCheckCount>=16?this._retryStreamLoad():this._scheduleStreamCheck()},500)}_retryStreamLoad(){this._clearStreamCheck(),this._streamCheckCount=0,this._streamRetryCount>=2?this._scheduleStreamCheck():(this._streamRetryCount+=1,this._streamLoaded=!1,this._streamEpoch+=1)}_clearStreamCheck(){this._streamCheckTimer&&(clearTimeout(this._streamCheckTimer),this._streamCheckTimer=void 0)}_onStreamLoad(){this._streamLoaded=!0,this._streamCheckCount=0,this._streamRetryCount=0,this._clearStreamCheck()}render(){if(!this.hass||!this._config)return q`<ha-card><div class="header"><span class="device-name">Nanit</span></div></ha-card>`;const t=this._entities(),e=this._isCameraOn(t);!e&&this._streamLoaded&&(this._streamLoaded=!1,this._streamCheckCount=0,this._streamRetryCount=0,this._clearStreamCheck());const i=t.camera?function(t,e){const i=t.states[e];return i?(i.attributes.friendly_name??"Nanit").replace(/ Camera$/i,""):"Nanit"}(this.hass,t.camera):"Nanit";return q`
      <ha-card>
    ...
    `}_renderHeader(t,e,i){const s=!this._config.hide_connectivity_status&&(bt(this.hass,e.wifi_ssid)||bt(this.hass,e.wifi_signal)||bt(this.hass,e.wifi_frequency)),n=!this._config.hide_baby_name,r=e.power&&!this._config.hide_power_button;return n||!i||s||r?q`
      <div class="header">
        ${n?q`
    ...
    `}_renderSensorOverlays(t){const e=[],i=this._config.temperature_entity_id||t.temperature,s=this._config.humidity_entity_id||t.humidity;if(bt(this.hass,i)){const t=parseFloat(this.hass.states[i].state),s=isNaN(t)?this.hass.states[i].state:t.toFixed(1),n=this.hass.states[i].attributes.unit_of_measurement??"";e.push(q`
        <div class="pill pill-temp" @click=${()=>this._fireMoreInfo(i)}>
          <ha-icon icon="mdi:thermometer"></ha-icon>
          <span>${s}${n}</span>
        </div>
      `)}if(bt(this.hass,s)){const t=parseFloat(this.hass.states[s].state),i=isNaN(t)?this.hass.states[s].state:t.toFixed(1),n=this.hass.states[s].attributes.unit_of_measurement??"%";e.push(q`
        <div class="pill pill-humid" @click=${()=>this._fireMoreInfo(s)}>
          <ha-icon icon="mdi:water-percent"></ha-icon>
          <span>${i}${n}</span>
        </div>
      `)}return 0===e.length?q``:q`<div class="overlay-top">${e}</div>`}_renderDetectionOverlays(t){const e=bt(this.hass,t.motion),i=bt(this.hass,t.sound);if(!e&&!i)return q``;const s=e&&"on"===this.hass.states[t.motion].state,n=i&&"on"===this.hass.states[t.sound].state;return q`
