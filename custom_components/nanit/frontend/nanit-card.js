...
`;let _t=class extends at{setConfig(t){this._config={...t}}_entityChanged(t){const e=t.detail.value;if(!this._config||e===this._config.camera_entity_id)return;const i={...this._config,camera_entity_id:e};this._config=i,this.dispatchEvent(new CustomEvent("config-changed",{bubbles:!0,composed:!0,detail:{config:i}}))}render(){return this.hass&&this._config?B`
      <div class="editor">
        <ha-entity-picker
          .hass=${this.hass}
  ...
  `,t([pt({attribute:!1})],_t.prototype,"hass",void 0),t([ut()],_t.prototype,"_config",void 0),_t=t([lt("nanit-card-editor")],_t);let vt=class extends at{static getConfigElement(){return document.createElement("nanit-card-editor")}static getStubConfig(t){const e=Object.keys(t.states).find(e=>e.startsWith("camera.")&&"nanit"===t.entities[e]?.platform);return{type:"custom:nanit-card",camera_entity_id:e||""}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config=t}getCardSize(){return 5}_entities(){return this._config?.camera_entity_id&&this.hass?function(t,e){const i={};i.camera=e;const s=t.entities[e];if(s?.device_id){const e=s.device_id,r=[];for(const i of Object.keys(t.entities))t.entities[i].device_id===e&&r.push(i);gt(i,r)}else{const s=e.split(".")[1]??"";gt(i,Object.keys(t.states).filter(t=>t!==e&&t.split(".")[1]?.startsWith(s.split("_camera")[0]||s)))}return i}(this.hass,this._config.camera_entity_id):{}}_isCameraOn(t){return!t.power||"on"===this.hass.states[t.power]?.state}_fireMoreInfo(t){this.dispatchEvent(new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:t}}))}_toggleService(t,e,i){this.hass.callService(t,e,{entity_id:i})}render(){if(!this.hass||!this._config)return B`<ha-card><div class="header"><span class="device-name">Nanit</span></div></ha-card>`;const t=this._entities(),e=this._isCameraOn(t),i=t.camera?function(t,e){const i=t.states[e];return i?(i.attributes.friendly_name??"Nanit").replace(/ Camera$/i,""):"Nanit"}(this.hass,t.camera):"Nanit";return B`
    ...
    `}_renderSensorOverlays(t){const e=[];if(mt(this.hass,t.temperature)){const i=this.hass.states[t.temperature].state,s=this.hass.states[t.temperature].attributes.unit_of_measurement??"";e.push(B`
        <div class="pill pill-temp" @click=${()=>this._fireMoreInfo(t.temperature)}>
          <ha-icon icon="mdi:thermometer"></ha-icon>
          <span>${i}${s}</span>
        </div>
      `)}if(mt(this.hass,t.humidity)){const i=this.hass.states[t.humidity].state;e.push(B`
        <div class="pill pill-humid" @click=${()=>this._fireMoreInfo(t.humidity)}>
          <ha-icon icon="mdi:water-percent"></ha-icon>
          <span>${i}%</span>
        </div>
      `)}if(mt(this.hass,t.light)){const i=this.hass.states[t.light].state;e.push(B`
        <div class="pill pill-light" @click=${()=>this._fireMoreInfo(t.light)}>
          <ha-icon icon="mdi:brightness-5"></ha-icon>
          <span>${i} lx</span>
        </div>
      `)}return 0===e.length?B``:B`<div class="overlay-top">${e}</div>`}_renderDetectionOverlays(t){const e=mt(this.hass,t.motion),i=mt(this.hass,t.sound);if(!e&&!i)return B``;const s=e&&"on"===this.hass.states[t.motion].state,r=i&&"on"===this.hass.states[t.sound].state;return B`
      ...
      <div class="control-section">
        ...
        ${r.length>0?B`
              <div class="source-list">
                ${r.map(e=>B`
                    <button
                      class="source-chip ${e===s?"active":""}"
                      @click=${()=>this.hass.callService("media_player","select_source",{entity_id:t,source:e})}
                    >
                      ${this._formatSourceName(e)}
                    </button>
                  `)}
              </div>
            `:V}
      </div>
    `}_formatSourceName(t){return t.replace(/\.wav$/i,"").replace(/_/g," ")}};vt.styles=ft,t([pt({attribute:!1})],vt.prototype,"hass",void 0),t([ut()],vt.prototype,"_config",void 0),vt=t([lt("nanit-card")],vt),window.customCards=window.customCards||[],window.customCards.push({type:"nanit-card",name:"Nanit Camera",description:"Camera stream with controls for Nanit baby cameras",preview:!0});export{vt as NanitCard};
