...
`;let wt=class extends ot{setConfig(t){this._config={...t}}_entityChanged(t,e){const i=e.detail.value||void 0;this._config&&i!==this._config[t]&&this._updateConfig({[t]:i})}_toggleChanged(t,e){const i=e.target.checked;this._updateConfig({[t]:i})}_updateConfig(t){const e={...this._config,...t};for(const[i,s]of Object.entries(t))void 0===s&&delete e[i];this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{bubbles:!0,composed:!0,detail:{config:e}}))}render(){return this.hass&&this._config?q`
  ...
  `,t([pt({attribute:!1})],wt.prototype,"hass",void 0),t([ut()],wt.prototype,"_config",void 0),wt=t([lt("nanit-card-editor")],wt);const yt=6e4;let $t=class extends ot{constructor(){super(...arguments),this._streamLoaded=!1,this._showNetwork=!1,this._streamEpoch=0,this._lastVideoTime=0,this._sawProgress=!1,this._stallStrikes=0,this._startupStrikes=0,this._healthyTicks=0,this._reloadCount=0,this._reloadWindowStart=0,this._cooldownUntil=0,this._streamMountedAt=0,this._watchedEpoch=-1,this._recoveringStream=!1,this._resumeRecoverUntil=0,this._startupReloads=0,this._wasCameraOn=!0,this._recoverStreamOnResume=()=>{if(document.hidden)return;const t=this._entities();if(!this._isCameraOn(t))return;if(!this.renderRoot.querySelector("ha-camera-stream"))return;const e=Date.now();e<this._resumeRecoverUntil||(this._resumeRecoverUntil=e+yt,this._recoverStream())}}static getConfigElement(){return document.createElement("nanit-card-editor")}static getStubConfig(t){const e=Object.keys(t.states).find(e=>e.startsWith("camera.")&&"nanit"===t.entities[e]?.platform);return{type:"custom:nanit-card",camera_entity_id:e||""}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config=t}getCardSize(){return 5}_entities(){return this._config?.camera_entity_id&&this.hass?function(t,e){const i={};i.camera=e;const s=t.entities[e];if(s?.device_id){const e=s.device_id,r=[],n=[];for(const i of Object.keys(t.entities))t.entities[i].device_id===e&&("diagnostic"===t.entities[i].entity_category?n.push(i):r.push(i));vt(i,r,t),function(t,e,i){for(const s of e){const[e]=s.split(".",1),r=s.split(".")[1]??"",n=i.states[s]?.attributes.device_class;"sensor"===e&&("signal_strength"===n?t.wifi_signal=s:"frequency"===n?t.wifi_frequency=s:r.endsWith("_wifi_ssid")&&(t.wifi_ssid=s))}}(i,n,t)}else{const s=e.split(".")[1]??"";vt(i,Object.keys(t.states).filter(t=>t!==e&&t.split(".")[1]?.startsWith(s.split("_camera")[0]||s)),t)}return i}(this.hass,this._config.camera_entity_id):{}}_isCameraOn(t){return!t.power||"on"===this.hass.states[t.power]?.state}_fireMoreInfo(t){this.dispatchEvent(new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:t}}))}_toggleService(t,e,i){this.hass.callService(t,e,{entity_id:i})}connectedCallback(){super.connectedCallback(),document.addEventListener("visibilitychange",this._recoverStreamOnResume),window.addEventListener("pageshow",this._recoverStreamOnResume),window.addEventListener("focus",this._recoverStreamOnResume)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("visibilitychange",this._recoverStreamOnResume),window.removeEventListener("pageshow",this._recoverStreamOnResume),window.removeEventListener("focus",this._recoverStreamOnResume),this._clearStreamWatchdog()}updated(t){super.updated(t);this.renderRoot.querySelector("ha-camera-stream")?(this._watchedEpoch!==this._streamEpoch&&(this._watchedEpoch=this._streamEpoch,this._streamMountedAt=Date.now()),this._streamWatchdog||this._startStreamWatchdog()):this._clearStreamWatchdog()}_startStreamWatchdog(){this._streamWatchdog=setInterval(()=>this._checkStreamLiveness(),1e3),this._checkStreamLiveness()}_clearStreamWatchdog(){this._streamWatchdog&&(clearInterval(this._streamWatchdog),this._streamWatchdog=void 0)}_findStreamVideo(t){const e=t.shadowRoot;if(!e)return null;const i=e.querySelector("video");if(i)return i;const s=e.querySelector("ha-hls-player, ha-web-rtc-player"),r=s?.shadowRoot?.querySelector("video");if(r)return r;for(const t of Array.from(e.querySelectorAll("*"))){const e=t.shadowRoot?.querySelector("video");if(e)return e}return null}_checkStreamLiveness(){const t=this.renderRoot.querySelector("ha-camera-stream");if(!t)return;const e=this._findStreamVideo(t);return!this._streamLoaded&&this._streamMountedAt>0&&Date.now()-this._streamMountedAt>1e4&&(this._streamLoaded=!0),e?(!this._streamLoaded&&e.readyState>=2&&(this._streamLoaded=!0),e.readyState<2?(this._startupStrikes+=1,void(this._startupStrikes>=20&&this._handleStartupFailure())):e.currentTime>this._lastVideoTime+.05?(this._lastVideoTime=e.currentTime,this._sawProgress=!0,this._stallStrikes=0,this._startupStrikes=0,this._startupReloads=0,this._streamLoaded=!0,void(!e.paused&&++this._healthyTicks>=10&&(this._reloadCount=0,this._cooldownUntil=0,this._reloadWindowStart=0))):(this._healthyTicks=0,void(this._sawProgress&&!e.paused&&(this._stallStrikes+=1,this._stallStrikes>=8&&this._recoverStream())))):(this._startupStrikes+=1,void(this._startupStrikes>=20&&this._handleStartupFailure()))}_requestBackendStreamReset(){const t=this._entities();return t.camera?this.hass.callService("nanit","reset_stream",{entity_id:t.camera}):Promise.resolve()}async _recoverStream(){if(!this._recoveringStream){this._recoveringStream=!0;try{await this._requestBackendStreamReset()}finally{this._recoveringStream=!1}this._reloadStream()}}_handleStartupFailure(){this._startupReloads+=1,this._startupReloads>=2?this._recoverStream():this._reloadStream()}_reloadStream(){const t=Date.now();t<this._cooldownUntil?this._stallStrikes=0:(t-this._reloadWindowStart>yt&&(this._reloadWindowStart=t,this._reloadCount=0),this._reloadCount+=1,this._reloadCount>=3&&(this._cooldownUntil=t+yt),this._streamEpoch+=1,this._streamLoaded=!1,this._lastVideoTime=0,this._sawProgress=!1,this._stallStrikes=0,this._startupStrikes=0,this._healthyTicks=0)}_onStreamLoad(){this._streamLoaded=!0}_resetStreamState(){this._streamLoaded=!1,this._clearStreamWatchdog(),this._lastVideoTime=0,this._sawProgress=!1,this._stallStrikes=0,this._startupStrikes=0,this._startupReloads=0,this._healthyTicks=0,this._reloadWindowStart=0,this._resumeRecoverUntil=0,this._streamMountedAt=0,this._watchedEpoch=-1}willUpdate(t){if(super.willUpdate(t),!this.hass||!this._config)return;const e=this._isCameraOn(this._entities());!e&&this._wasCameraOn&&this._resetStreamState(),this._wasCameraOn=e}render(){if(!this.hass||!this._config)return q`<ha-card><div class="header"><span class="device-name">Nanit</span></div></ha-card>`;const t=this._entities(),e=this._isCameraOn(t),i=t.camera?function(t,e){const i=t.states[e];return i?(i.attributes.friendly_name??"Nanit").replace(/ Camera$/i,""):"Nanit"}(this.hass,t.camera):"Nanit";return q`
      <ha-card>
